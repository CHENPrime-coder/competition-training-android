...
class MembersChangeHeadPage extends StatefulWidget {
  MembersChangeHeadPage({Key? key}) : super(key: key);

  @override
  State<MembersChangeHeadPage> createState() => _MembersChangeHeadPageState();
}

class _MembersChangeHeadPageState extends State<MembersChangeHeadPage> {
  late File file;
  bool isSelect = false;
  String tips = "请选择上传头像";
  ...
  changeUserImg() async {
    ...
    String oldFilename = CurrentStudio.studio["sheadicon"].toString();
    setState(() {
      isLoading = true;
    });
    dynamic res = await StudioDio.changeHeadIcon(newFilename,oldFilename,CurrentStudio.studio["sname"]);
    if(res["code"]=="200"){
